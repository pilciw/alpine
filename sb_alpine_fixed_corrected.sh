...
if [[ x\"${release}\" == x\"alpine\" ]]; then
    apk update
    apk add --no-cache wget curl tar jq tzdata openssl expect git socat iproute2 iptables grep coreutils util-linux dcron procps qrencode
apk add virt-what
apk add qrencode
else
...
if [[ x"${release}" == x"alpine" ]]; then
    mkdir -p /var/run
    cat > /etc/s-box/sb-service.sh <<'EOF'
#!/bin/sh
PID_FILE="/var/run/sing-box.pid"
CONFIG_FILE="/etc/s-box/sb.json"
BIN_FILE="/etc/s-box/sing-box"
...
EOF
    chmod +x /etc/s-box/sb-service.sh
    /etc/s-box/sb-service.sh start
else
